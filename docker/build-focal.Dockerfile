...
RUN set -e \
    ...
    && apt-get -y install appstream autoconf automake autopoint build-essential cmake git libass-dev libbz2-dev libfontconfig1-dev libfreetype6-dev \
        ...
        libjansson-dev git debhelper-compat yasm distcc ccache wget libmfx-dev clang curl libssl-dev libcurl4-openssl-dev python3-pip \
    && python3 -m pip install --no-cache-dir --upgrade meson \
    && rm -rf /var/lib/apt/lists/* \
    && rm -rf /tmp/* /var/tmp/* /var/log/*

# HandBrake 1.11.2+ requires autoconf 2.71; focal only has 2.69
RUN set -e \
    && apt-get update \
